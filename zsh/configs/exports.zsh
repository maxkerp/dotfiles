# FZF Base16 Tomorrow Night Theme
# Big thankgs to the following sources:
#
# https://minsw.github.io/fzf-color-picker/
# https://github.com/chriskempson/base16-tomorrow-scheme/blob/master/tomorrow-night.yaml
# http://chriskempson.com/projects/base16/
export FZF_DEFAULT_OPTS=' --color=fg:#969896,bg:#1d1f21,hl:#b5bd68 --color=fg+:#e0e0e0,bg+:#1d1f21,hl+:#b5bd68 --color=info:#81a2be,prompt:#cc6666,pointer:#cc6666 --color=marker:#b294bb,spinner:#8abeb7,header:#f0c674'

# For elsa Project
export CAMUNDA_DIR='/home/taktsoft/Projects/FBB/Downloads/camunda-bpm-tomcat-7.11.0/server/apache-tomcat-9.0.19/bin'

# Use nvim for manpages
export MANPAGER='nvim +Man!'

